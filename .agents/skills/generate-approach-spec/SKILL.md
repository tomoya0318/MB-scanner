...
argument-hint: <スコープ: ディレクトリパス=README / アプローチ名=仕様書> [mode=paper|overview（仕様書で実測値を載せる時のみ。既定 overview）]
---
...
2. コミット印を取得する: `git rev-parse --short HEAD` と当日日付。この印は **生成直前の HEAD**
   (= この生成物を収録するコミットの親) であり、生成物自身のコミットハッシュではない
   (生成 → 検証 → コミットの順で、自身のハッシュは印字時点では未確定)。
...
- 他ディレクトリのファイルを指すパス・引用は **その文書のディレクトリ起点の相対表記** に統一する
  (例: `preprocessing/README.md` から契約を指すなら `../contracts/preprocessing-contracts.ts`)。
  裸の `contracts/...` は文書起点で実在しないパスに解決するため不可 (手順 3 の照合対象)。
...
README には実測値を書かないため mode は無関係。仕様書に実測値が必要なときだけ mode を使い分ける
(引数省略時は `overview`):
