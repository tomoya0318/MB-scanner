...
| [0022](0022-preprocess-workload-reachability.md) | preprocess を workload-reachability ベースに再設計し `candidate_kind: changed-fn` を追加 (v1) | accepted (`superseded by 0023` 予定) | `mb-analyzer/src/preprocessing/{common,selakovic}/` |
| [0023](0023-preprocess-placeholder-substitution.md) | preprocess を placeholder substitution + 4 値契約に書き直す (v2) | accepted | `mb-analyzer/src/preprocessing/selakovic/`, `mb-analyzer/src/contracts/{preprocessing,equivalence,pruning}-contracts.ts` |
| [0024](0024-preprocess-contract-base-adapter-split.md) | preprocess contract を base / adapter 分離 + issue 階層化に再設計する | accepted | `mb-analyzer/src/contracts/`, `mb_scanner/src/mb_scanner/domain/entities/preprocessing.py`, `mb-analyzer/src/preprocessing/selakovic/` |
| [0025](0025-server-strategy-commonjs.md) | server SUT を CommonJS-respecting holed lib + node:vm 直 eval で扱う | accepted (ローカル spike 3/3 equal: chalk-27a / chalk-28 / cheerio-386b) | `mb-analyzer/src/preprocessing/selakovic/assemble/strategies/server-changed-fn.ts` (順 3-2 で新規), `mb-analyzer/src/equivalence-checker/common/sandbox/executors/vm.ts` |
| [0027](0027-changed-fn-rename-collision-guard.md) | changed-fn rename-only 救済の collision guard (scope-aware rename 不採用、ADR-0023 D-γ §順 1-b の安全弁) | accepted | `mb-analyzer/src/ast/inspect.ts`, `mb-analyzer/src/preprocessing/selakovic/assemble/strategies/changed-fn.ts` |
| [0028](0028-pruning-protect-diff-literals.md) | pruning 差分フィルタで差分サブツリー内のリテラルを保護 (親も共通な時のみ wildcard 候補)。既知の限界 2 件 (繰り返し式の hash 衝突 / 挿入型最適化) を併記 | accepted | `mb-analyzer/src/pruning/common/candidates.ts` |
