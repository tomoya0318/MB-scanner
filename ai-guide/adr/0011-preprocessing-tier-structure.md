...
- **Date**: 2026-05-10
- **Related**: ADR-0010 (Tier 1 の enclosure 戦略), ADR-0013 (Tier 2 が落とす harness は等価判定でも非観測), ADR-0014 (両側変化時の candidate 分割 = 本 ADR §段 2 の「両方差あり」分岐), ADR-0015 (等価検証器も同じ common/adapter 二層), ADR-0016 (Tier 2 が特定した `<lib>_*.js` が require する npm dep は dataset fork に lockfile で宣言して解決), ADR-0017 (preprocess は loop bound を保ち sandbox が iteration-cap をかける ← 本 ADR §段 1 の「ループ反復回数は書き換えない」と対), `mb-analyzer/src/preprocessing/{common,selakovic}/`, `mb-analyzer/src/contracts/preprocessing-contracts.ts`, `ai-guide/code-map.md` §Selakovic 前処理器, `ai-guide/datasets/selakovic-2016-issues.md` (§`{setup, slow, fast}` 対応表 — 本 ADR で clientIssues も `<script src>` 解決で `<lib>_*.js` を diff 対象にするよう更新が要る), `tmp/patch-taxonomy.md` §1/§5/§7 (作用点 A/B の定義), `tmp/case-split-feasibility.md` (5 ケース統合), `tmp/dataset-conventions.md` §2.4/§2.5/§5, `tmp/0002_phase1-adr-and-spike/plan.md` Phase 1.1
...
- **server lib の npm dep**: server 系の `<lib>_*.js` (例 `chalk_before.js`) は `require('escape-string-regexp')` 等の npm dep を bundle していない (Selakovic dataset の制約 — `spike-results.md` §6)。`init()` がその lib を `require` する issue は実行時にその dep が要る → 上流が宣言していないので **dataset fork に dep を lockfile で宣言して解決する** (ADR-0016。`package.json` + `pnpm-lock.yaml` を同梱し `pnpm install` で `node_modules/` を再生成、checker 側は実装変更ゼロ)。preprocess 自体は dep が無くても `<lib>_*.js` の diff は取れる (parse はできる) が、実行する equivalence-checker は dep を要する。
...
### 2026-05-12 更新 (作用点ラベルのリネーム + lib narrowing の再設計 = 0022)

§段2 の作用点ラベル `A` / `B` / `A+B` を **`lib` / `workload` / `lib+workload`** にリネームした (本文中の `作用点 A`/`作用点 B`/`作用点 A+B` は読み替え)。`A`/`B` が opaque で、lib narrowing の再設計 (`tmp/0022_preprocess-workload-reachability-redesign/`) で「workload (`f1`/`test()`) が exercise する変更関数だけを候補に絞る」という枠組みを採ったので、用語を `workload` に揃えた。実 PR に拡張するときの `lib` (= production コード変更、常態) ↔ `workload` (= test/workload コード変更、稀) の二分法にもなる。contract: `mb-analyzer/src/contracts/preprocessing-contracts.ts` の `ASPECT` (`"lib"` / `"workload"` / `"lib+workload"` / `"fallback"`) + Python `Aspect`。

§段2 の「作用点 A → `<lib>_*.js` の変更関数 body を slow/fast に」は 0021 で `lib-enclosure` candidate (= 変更を全部含む最小 enclosure を `<lib>_*.js` から切り出す) として実装したが、変更が IIFE 内の複数兄弟関数に散ると enclosure = IIFE 全体 (Ember で ~9 万ノード) になり縮まらないと判明 → **0022 で「workload が (推移的に) exercise する変更関数を 1 つずつ取り出して lambda-lift + 観測する形にする」(`candidate_kind: "changed-fn"`) に再設計**。`candidate_kind` から `lib-enclosure` は削除、`changed-fn` を追加 (`EXCLUSION_REASON` に `change-not-exercised` も)。詳細は `tmp/0022_.../plan.md` / `notes.md`、equiv-checker 側の関連修正は ADR-0018 の「2026-05-12 更新」。

### 2026-05-15 更新 (ADR-0024 で base/adapter 分離)

`aspect` フィールドは ADR-0024 で `SelakovicIssueMeta.aspect` (issue level の adapter 拡張) に移動する。本 ADR の Tier 構造 (Tier 1 + Tier 2) と作用点ルーティングの設計自体は不変。詳細は ADR-0024 §決定 を参照。
