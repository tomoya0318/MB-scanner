...
- `common/verdict.ts`: verdict 合成規則 — **現行は ADR-0018 の新 5 規則** (`not_equal` あり → `not_equal` / `error` あり → `error` / 全 N/A → `inconclusive` / positive-evidence oracle `{return_value, argument_mutation, interaction_trace}` が全 N/A → `inconclusive` / それ以外 → `equal`) + `deriveVerdictReason`。ADR-0013 にあった旧 4 規則 (全 N/A → `error` / N/A 以外すべて `equal` → `equal`) は ADR-0018 で置き換え済。
...
| console ノイズ除去 + new_globals 除外 | 計測ハーネスの `console.log(mean)` 等が harness 除去を擦り抜けた場合に O4 入力から除く既知パターン (保険) / **`new_globals` から除外する正規表現群** (`/^ng/` = AngularJS 内部 global、`/^__selakovic/` = runnable scaffolding、`/^[a-z]$/` + よくある loop/temp 変数名 = `f1` body/preF1 が `<script>` で top-level 実行されるため `var i`/`for(var keys ...)` 等が global に漏れる artifact — underscore-1224 の偽 not_equal 解消) | O4 `external-observation.ts` の `opts.ignoreNewGlobalPatterns` |
| exception message 正規化 | dataset が slow/fast を `<lib>_before/`/`<lib>_after/` の別 dir に置くので dep 解決失敗の error message に `Cannot find module './backbone_before/...'` 等の配置 artifact が混じる → 比較前に `_(before\|after)` を除去 (backbone-1097/2858/707 / mocha-763 の偽 not_equal 解消) | O3 `exception.ts` の `opts.normalizeMessagePatterns` |
| sandbox config | timeout (重い AngularJS issue 用に長め、97 件は 20000ms で実走) / stub する非決定性 API のリスト / **iteration-cap の `{ threshold, cap }`** (リテラル上限が `threshold` 以上のループを `cap` 回に clamp。当初 threshold=10000 だったが「数百回ループの 1 反復ごとの C6 trace × 反復回数が記録 Proxy の trace 上限 2000 を超え、打ち切り位置が slow/fast でずれて偽 not_equal」(moment-1885) を避けるため threshold=100 / cap=5 に下げた。`cap=null` で無効化 = 原文どおり全反復。transform の*アルゴリズム*は ADR-0017) | `common/sandbox/executor.ts` + `common/sandbox/transforms/iteration-cap.ts` |
...
- **sandbox 側 (= 汎用記録 Proxy の機構)**: `executor.ts` が adapter から渡された「包む対象」のオブジェクトを **`get`/`set`/`apply`/`construct` トラップを持つ記録 Proxy** で wrap し、呼び出しを `capture.interaction_trace` に append する。`get`/`set`/`apply`/`construct` 以外のトラップは未定義 (= Reflect 素通し) なので `instanceof` / `hasOwnProperty` / `Object.getPrototypeOf` / `ownKeys` を壊さない (Selakovic の 7 SUT どれも Proxy で包んで壊れなかった)。`apply`/`construct` トラップは「SUT を関数として直接呼ぶ」(cheerio root `obj(...)`、`jQuery(...)`、`ejs(...)`) や「コンストラクタ」(`new LRUCache(...)`) を記録するのに要る。同一 target の再 wrap は WeakMap でキャッシュ (chained API で identity が暴れるのを防ぐ)。oracle はできた列を受け取るだけ = 実行環境非依存。当初案の「`get`/`set` だけの Proxy」では cheerio の連鎖や jQuery の関数呼び出しが取れないので不足 — Phase 2b 着手前 spike で確定 (`spike-results.md` §3/§7)。
  - **trace / serialize から除外するもの** (= SUT 内部レイアウトやプロトコル機構であって「workload↔SUT の観測可能な interaction」ではないノイズ。97 件再走で詰めた値): プロパティキーの prefix `["$$", "_"]` (AngularJS の `$$hashKey`、多くの lib が「private/内部」に使う `_`-prefix — Backbone/moment/request 等の内部レイアウト差を消す) + exact `["cid"]` (Backbone の連番 ID) を serialize と get-trace から除外 / `valueOf`/`toString`/`toLocaleString`/`toJSON`/`then`/`Symbol.toPrimitive`/`Symbol.iterator`/`Symbol.asyncIterator` は「interaction でなく値変換/イテレーション/Promise 機構」なので get-trap で raw 関数を real target に bind して返し呼んでも記録しない (`""+wrappedObj` の `valueOf`/`toString`・`for..of` の `Symbol.iterator` を trace に乗せると moment の `isAfter` が before: `other._a` read / after: `other.valueOf()` call で偽 not_equal を生むのを防ぐ) / **lib ごとの recurse 設定**: 既定は `recurse:true` (= traced call/construct の戻り値を再帰 wrap して chained API を辿る) だが、jQuery (`$`/`jQuery`) は `$(...).children().domManip(...)` の chain を内部多用し wrapped jQuery object を `domManip` が `.length`/`.name`/`getPrototypeOf` 経由で触って壊れる (jquery-200) ので top-level の `$`/`jQuery` だけ wrap し chain は wrap しない (`recurse:false`)。/ trace 上限 2000 entries / 1 値 16KB / 深さ 6 / 幅 24 (react-895 の component-tree 構築で trace が爆発しプロセス fatal するのを防ぐ。iteration-cap の threshold を 100 に下げたので実際にはほぼ当たらない)。
...
- **記録 Proxy が SUT を perturb する false positive が残る** (= ADR-0013 §諦めるもの の「C6 の記録 Proxy が SUT を perturb する」と同じ): `recurse:false` で jQuery の chain を wrap しないようにしても `$` 自体を Proxy にした時点で version-sensitive な内部が壊れるケース (jquery-200) / SUT が返す rich stateful object の内部構造が trivial に differ するケース (request-1165 の `Request` object の HTTP agent) は残る。`not_equal` のうちこの class は Phase 3 の手動レビューで「記録 Proxy 干渉」として角に置く。根本対策は「SUT に渡された wrapped object の内部アクセスを記録しない (provenance 追跡)」だが該当数が少ないので未実装 — トリガーで監視
...
oracle 選択は本 ADR で並記された 2 案 (「hint で部分集合を選ぶ」案 vs 「N/A 自己判定で over-list は verdict 不変」案) のうち、後者 (`oracle-routing.ts:30-32` の `environment` 1 軸) を正式裁定として ADR-0024 で確定した。preprocess 由来 hint (`aspect` / `candidate_kind` / `enclosure_type`) は `EquivalenceInput` / `PruningInput` から廃止する (実コードでは元々参照ゼロ = dead code 削除)。DOM 正規化プロファイルが framework ごとに分岐するトリガー発火時には、selakovic adapter 内で `(aspect, target_side, layout, wrapper_kind)` から派生する lookup を持つ形で対応する (本 ADR の二層化方針は不変)。詳細は ADR-0024 §決定 を参照。
