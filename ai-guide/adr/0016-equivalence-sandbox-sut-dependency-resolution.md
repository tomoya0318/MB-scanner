...
### 2026-05-12 更新 (本 ADR の仕組みを client `<script src>` CDN dep にも適用 — 0022 Phase 3)

本 ADR の守備範囲は当初「SUT lib / `init()` が `require` する **server 側 npm dep**」だったが、同じ仕組み (fork の `package.json` + `pnpm-lock.yaml` + `scripts/install-vendor-deps.sh`) を **client issue (`v_*.html`) が `<script src>` で CDN から読む依存 lib (jquery / handlebars / underscore)** にも適用する。fork PR #2 (`be15a06`) で client-issue 用の vendor location を追加 (`clientIssues/AngularIssues` = jquery@1.11.3 / `clientIssues/EmberIssues` = jquery@2.1.3+handlebars@1.1.0、issue_3174/3288 は jquery@1.7.2 に override / `clientIssues/ReactIssues` = jquery@1.7.2 / `clientServerIssues/{Backbone(underscore@1.8.3+jquery@2.1.3), Ejs, Moment, NodeLruCache, Q(jquery@1.7.2), Underscore.string, Underscore}` = jquery@2.1.3)。jquery 自体が SUT の `JQueryIssues` は `<script src="jquery_before.js">` (dataset-local) なので vendor 不要。

**server `require()` dep との違い (= 解決の仕方)**: server 側は等価検証 sandbox が `createRequire(moduleBaseDir)` で引く (本 ADR の本題)。client 側の `<script src>` dep は **jsdom が `<script src>` を auto-load しない**ので、MB-scanner の **preprocess** が `node_modules/<pkg>/dist/<pkg>.js` を読んで候補の `setup` 先頭に連結する (`mb-analyzer/src/preprocessing/selakovic/io/script-deps.ts` の `resolveScriptDepSources` — issueDir から祖先方向に最寄りの `node_modules/<pkg>` を解決、issue 単位が category 単位より優先 → issue_3174/3288 の jquery 1.7.2 override が効く。`pipeline.ts` が `dep_lib_sources` を全候補の `setup` に連結 — plan §C1)。executor (`equivalence-checker`) は不変。

Ember 4158 (`v_before.html` が Ember 1.5 と非互換な jquery 2.1.3 を `<script src>` する dataset bug) は issue 単位 override を入れてないので `EmberIssues` category の jquery 2.1.3 が解決され、Ember bootstrap が assert で落ちる → equiv gate で `error` → discard (recall 限界。必要なら後で issue_4158 にも jquery 1.7.x の override を入れて救済できる)。

トリガー発火ではない (= 方式 A のまま、適用範囲を広げただけ) ので superseded にはしない。`MODIFICATIONS.md` (fork) も更新済。
