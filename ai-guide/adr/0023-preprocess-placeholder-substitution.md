...
(globalThis.__OBS__ = globalThis.__OBS__ || []).push((function () {
...
```ts
// slow 評価:
const slowSetup = substituteBody(setup, slow);   // setup の $BODY$ に slow (= observed before body) を差し込む
await executeSandboxed({ setup: slowSetup, body: workload, timeout_ms });

// fast 評価 (= 別 sandbox):
const fastSetup = substituteBody(setup, fast);
