...
- **B. placeholder substitution + 4 値契約 (= 本 ADR の提案、v2)**: `setup` に `$BODY$` プレースホルダ、`workload` に観測 hook + f1 body、`slow`/`fast` に before/after body の statement 列のみ。executor は **既存の 2 引数 (setup, workload) API に展開** (= setup = `substituteBody(originalSetup, slow/fast)`, workload = workload フィールド) → 既存の 2 回 runInContext + snapshot 機構 (argument_mutation oracle 用) を再利用。
...
| フィールド | 役割 | 中身 |
|----------|------|-----|
| `setup` | sandbox で `workload` を実行する前に context を整える文字列。executor の `setup` 引数に渡す | `libs` + `preWorkload` の連結。具体形は下記 §sandbox 実行イメージ |
| `workload` | sandbox の completion value を返す式。executor の `body` 引数に渡す | dataset の workload 関数 body を `$BODY$` 経由で観測した結果を返す IIFE |
| `slow` / `fast` | 変更関数本体 (before / after) の **裸** statement 列 (`statementsToCode` 出力)。`setup` の `$BODY$` プレースホルダ (= 観測 IIFE 内側、`replaceFunctionBodyWithObserver` の出力で形成) に差し込まれる。観測ハーネスは setup 側に inline 化されるので slow/fast には載らない (D-δ §observation 仕様) | (`$BODY$` に差し込まれる裸 statement 列) |

#### sandbox 実行イメージ (= 全体像)

`setup` には変更関数本体が「観測 IIFE + `$BODY$` プレースホルダ」になった lib + workload 前置文が入る (= D-δ で観測ハーネスを setup 側に inline 化):

```js
// setup (= libs + preWorkload、観測 IIFE 入りの $BODY$ プレースホルダ含む)
//   libs:
!function (root, String) {
  ...
  var _s = {
    startsWith: function (str, starts, position) {
      // ↓ replaceFunctionBodyWithObserver で setup 側に inline 化された観測 IIFE (D-δ)
...
`slow` (および `fast`) は次のような **裸 statement 列** で `$BODY$` に差し込まれる (= 観測ハーネスは setup 側が担うので slow/fast には載らない):

```js
// $BODY$ (= 観測 IIFE 内側) に差し込まれる裸 body — statementsToCode の出力
...
`__OBS__` は **executor に渡す setup 引数の最先頭** に `let __OBS__ = [];` で宣言される (= `declareObservationGlobal` helper、sandbox top-level の lexical binding として `replaceFunctionBodyWithObserver` で inline 化された観測 IIFE / `wrapObservedWorkload` の closure から参照される)。top-level `let` の特性で `globalThis.__OBS__` 経由のアクセスは不可 = scope を跨いだ誤参照を仕様レベルで防ぐ。
...
await executeSandboxed({ setup: slowSetup, workload, timeout_ms });

// fast 評価 (= 別 sandbox):
const fastSetup = declareObservationGlobal(substituteBody(setup, fast));
await executeSandboxed({ setup: fastSetup, workload, timeout_ms });
```

`executeSandboxed` は内部で `setup` 引数と `workload` 引数を順に sandbox で実行し、間で setup state を snapshot して argument_mutation oracle 用に取る (= 既存実装、`equivalence-checker/common/sandbox/executors/{vm,jsdom}.ts`)。executor 側は無改修で placeholder model に対応する。
...
- **sandbox 実行時の internal 変数**: **`__NAME__`** (両端 underscore で囲む)
  ...
  - `__OBS__` は setup 最先頭の `let __OBS__ = [];` (= `declareObservationGlobal` helper) で宣言・初期化され、`replaceFunctionBodyWithObserver` で inline 化された観測 IIFE / `wrapObservedWorkload` の closure 経由で参照される (= `globalThis.` プレフィックスは不要)
  - `__OBS_R__` は `replaceFunctionBodyWithObserver` が setup 側に inline 化する観測 IIFE の `let __OBS_R__` で 1 関数 body 内に閉じる (D-δ §observation 仕様、D-β/D-γ では `wrapBodyObserved` で slow/fast 側に持っていたが本 ADR D-δ で setup 側へ移動)
...
### D-δ 観測ハーネス外出し (2026-05-18)

D-β/D-γ では `slow`/`fast` を `wrapBodyObserved` で観測 IIFE に包んでいたため、pruning が見る範囲に観測ハーネスが含まれ、抽出 pattern_code に等価検証の足場が残った (例: `function () { ... }.call("$P5"); $P0; return $P3;`)。これは「変更関数の等価性の核」ではなく、pattern として読みづらい。

本 D-δ で観測ハーネスを setup 側の関数本体に inline 化し、slow/fast は裸 body の statement 列に揃える:

- `placeholder.ts`: 新 helper `replaceFunctionBodyWithObserver` 追加 (= `{ $BODY$ }` の内側に観測 IIFE を inline 化、実装は span slicing で観測ハーネスを直接埋め込む = `source.slice(0, start) + "{ <observer> }" + source.slice(end)`、source 側のコメント / 文字列リテラルに `$BODY$` が紛れていても fail-loud に `substituteBody` が throw する)、旧 `wrapBodyObserved` を完全削除 (= 呼び出し元ゼロの dead code)
- `changed-fn.ts`: `holedLib` を `replaceFunctionBodyWithObserver` に差し替え、`slow`/`fast` から観測ラップを撤去 (= `statementsToCode` の裸 body)
- `equivalence-checker/selakovic/checker.ts`: ロジック無改修 (= `substituteBody(setup, slow=裸body)` で `$BODY$` = 観測 IIFE 内側に裸 body を差し込むので、最終実行コードは D-β/D-γ と同形)

**同形性**: B 案後の最終実行コードは D-β/D-γ と string レベルで同形 (= 観測 IIFE 内側に裸 body が入る = D-β/D-γ で slow/fast 全体が観測 IIFE になっていたのと等価)。verdict 数の baseline 一致 (= D-γ equal=25 / not_equal=6 / error=12) は Phase 3 全件再走で確認。

**pattern_code への効果**: pruning 入力 (slow/fast) から観測足場が消えるので、抽出 pattern_code には「変更関数の等価性の核」だけが残る。さらに同じ binding を指す identifier の placeholder 重複 (例: `obj × 3 → $P6/$P7/$P8`) は別途 research 後処理スクリプト `normalize_pattern.py` で統合 (= Phase 2)。
