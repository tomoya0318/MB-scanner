...
- `placeholder.ts`: 新 helper `replaceFunctionBodyWithObserver` 追加 (= `{ $BODY$ }` の内側に観測 IIFE を inline 化、合成パターンは `replaceFunctionBody` + 観測ハーネス string の 2 段階置換)、旧 `wrapBodyObserved` を完全削除 (= 呼び出し元ゼロの dead code)
