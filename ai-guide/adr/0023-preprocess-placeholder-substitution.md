...
### 4 値契約の具体形 (D-α spike を経た最終形 = body 観測注入)

D-α spike (2026-05-13) で「workload 側に observe hook を立てる」案 (= `_s.startsWith = function () {...}` で外から上書き) を試したところ、**`_s` は lib IIFE 内ローカル名で bootstrap 後の global からは `_.str` 等の別エイリアス経由でしか到達できず、`<name> = function () {...}` 形の上書きが ReferenceError or 別オブジェクト代入になる** 限界が判明。代わりに **body 内部に観測ラッパを注入** する形に修正した。これで access name の resolution 不要、closure dep 自動解決、bootstrap-invocation 経路も観測点として活用可能。
...
!function (root, String) {
  ...
  var _s = {
    startsWith: function (str, starts, position) {
      $BODY$   // ← 「観測する形」の slow body / fast body をテキスト置換で差し込む
    },
    /* ... */
  };
...
// slow / fast に差し込まれる body (= 観測する形) — wrapBodyObserved(originalBody) の出力
var __r = (function () {
...
// workload (= f1 body、slow/fast 共通、IIFE で completion value 化)
(function () {
  globalThis.__OBS__ = [];
  for (var i = 0; i <= 100000; i++) {
    _.str.startsWith('image.gif', 'image');   // ← bootstrap 後の alias で OK、access name 解決不要
  }
  return JSON.stringify(globalThis.__OBS__);
})()

// executor (jsdom / vm 共通) — 4 値を既存 executor の 2 引数 (setup, body) に展開して呼ぶ
//   = setup state snapshot 機構 (argument_mutation oracle 用) を維持できる
const slowSetup = substituteBody(originalSetup, slow);   // = lib + preF1 (slow body 差し込み済)
const slowResult = await executeSandboxed({ setup: slowSetup, body: workload, timeout_ms });
const fastSetup = substituteBody(originalSetup, fast);
const fastResult = await executeSandboxed({ setup: fastSetup, body: workload, timeout_ms });
```

`makeString` は lib IIFE の closure で見える → **lambda-lift 不要**。body 観測注入は元の関数の位置で実行されるので **`if (__HOLE__)` ガード不要** (bootstrap-invocation 中も同じ body が走るが、workload IIFE 先頭の `__OBS__ = []` reset で bootstrap 観測は捨て純粋な workload 観測だけが残る)。観測 hook (= access name resolution) は **不要** = AMD/IIFE 内ローカル名にも強い。

旧契約 (= `workload` フィールドなし、`single`/`lib`/`body` / fallback) は **後方互換**: そのまま既存 executor の (setup, body) に直接渡る。placeholder 経路と旧経路の差は呼び出し側 (`changed-fn.ts`) で setup を `substituteBody(originalSetup, slow/fast)` に展開するかどうかだけで、executor 側は無改修。
