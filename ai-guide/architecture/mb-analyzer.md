...
pruning/            ──→ ast, contracts, equivalence-checker
...
- **`ast/`**: 末端層。Babel AST 操作の汎用ユーティリティ (parse / walk / subtree-hash / inspect)。pruning と将来の機能で共有
...
│   ├── contracts/            # 末端層: 型定義のみ（他機能を import 禁止）
