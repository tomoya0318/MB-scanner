...
| Reference (生成型) | **Reference** — 〜する仕組み / 〜のため〜 | file:line 引用付きの生成文書 | 論文執筆、onboarding |
...
- **Reference 軸**: 維持型の `code-map.md` は廃止 (判断: [ADR-0029](../adr/0029-generated-reference-docs.md))。導出情報 (モジュール構成・ファイル責務・データフロー) は保存せず、in-tree README / アプローチ仕様書を skill で必要時に生成する。非導出情報は種類ごとの恒久の家に置く: 設計判断 → `adr/`、研究上の主張・既知の限界 → `current-research.md`、実測値 → `research/<approach>/notes/` (コミット印必須)、局所的な運用上の落とし穴 → 該当コードの現地コメント。
- **adr/**: 採用判断の根拠と却下した選択肢。1 ADR = 1 判断。現行契約は必ず `architecture/` 側に反映する (ADR は履歴扱い)。

### 実測値の二段運用 (ADR-0029)

- **論文モード**: `research/<approach>/code/` の再走スクリプトを、最新契約との整合確認込みで再実行した値のみを使う。
- **理解モード**: `research/<approach>/notes/` の保存値を使ってよいが、コミット印 (実行コミット・コマンド・日付) の併記を必須とする。
...
README は維持型ではなく **skill 生成物** (判断: [ADR-0029](../adr/0029-generated-reference-docs.md)): `generate-approach-spec` skill で生成し、コミット印付きスナップショットとして常設コミットする。**手編集は禁止** (次回生成で消える) — 修正したいときも skill で再生成する。鮮度はヘッダの生成時コミット印で機械的に判定する。
...
in-tree README が担うもの:

- ディレクトリ内**各ファイルの責務**
- ディレクトリ内**ファイル間の依存方向**
- モジュール固有の**契約の要約** (CLI なら argv / stdin / stdout / stderr / 終了コード)
- 関連 ADR の索引 (このディレクトリの判断履歴)

### 参照方向

- **NG**: in-tree README から `current-research.md` 等の **上位 / 流動的なドキュメント** への参照

理由: current-research.md は研究方針に応じて頻繁に更新されるのに対し、README はディレクトリ churn でしか更新されない。**頻繁に変わる側 → 安定している側** へのポインタのみを許すことで、README が drift するリスクを排除する。

README は **自己完結** させる: ファイル責務・依存方向・関連 ADR のみで意味が通り、上位文書を読まなくてもディレクトリの全体像が把握できる粒度を保つ。

### 生成形式

ヘッダ (生成物宣言 + 生成時コミット印) を含む出力形式・読込対象・検証パスは
`generate-approach-spec` skill (`.agents/skills/generate-approach-spec/SKILL.md`) が規定する (1 出典)。

### 再生成トリガー

**ディレクトリ内のファイル churn** が起きたときに skill で再生成する。

- ファイル追加 / リネーム / 削除
- 役割・契約の根本的な変更 (実装の細部変更では再生成不要)

→ 書く粒度を「役割・依存・契約」止まりにする (関数シグネチャや実装詳細は書かない) ことで、
実装の日々の変更ではスナップショットが陳腐化しないようにする。
...
```
architecture/  >  quality-check/  >  in-tree README (生成物)  >  adr/
契約             検証手順           リファレンス               履歴
```

- **契約 (architecture) が最優先**。README の記述が architecture/ と食い違う場合は architecture/ を信じる。
