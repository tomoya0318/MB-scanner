...
- **ADR への参照**: 採用判断を示すコメントは `// 判断: ai-guide/adr/NNNN-xxx.md` 形式で 1 行 (理由や却下案は ADR 側に)。契約文 (JSDoc / docstring / 説明コメント) 中のポインタは `(ADR-NNNN)` / `(ADR-NNNN §x)` の略記を許容する。いずれも `ADR-` prefix 付き番号で書く (裸番号・行番号参照は不可)
