...
- `mb-analyzer/src/pruning/` (engine / candidates / index + rules/ + ast/)
...
将来 mb_scanner/ 側でも対象が出れば追加。subdirectory レベル (`pruning/ast/` / `equivalence-checker/common/comparison/` / `preprocessing/selakovic/io/` 等) は親 README で記述するため不要。
