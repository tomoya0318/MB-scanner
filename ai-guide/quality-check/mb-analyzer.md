...
| **モジュール内共有ヘルパ** (ファイルから export はあるが `index.ts` に乗らない) | 同一 `src/` ファイル末尾の `if (import.meta.vitest)` ブロック | pruning: `enumerateCandidates` / `WHITELIST_CATEGORIES` / `BLACKLIST_CATEGORIES` / `replacementFor` → `src/pruning/{candidates, rules/whitelist, rules/blacklist, rules/replacement}.ts` 内; `parse` (whitelist-aware ラッパ) → `src/pruning/ast/parser.ts` 内; 共有 AST toolbox の `SubtreeSet` / `walkNodes` / `countNodes` / `snippetOfNode` → `src/ast/{subtree-hash, walk, inspect}.ts` 内。equivalence-checker: oracle 6 本 (`checkReturnValue` / `checkArgumentMutation` / `checkException` / `checkExternalObservation` / `checkDomMutation` / `checkInteractionTrace`) → `src/equivalence-checker/common/comparison/oracles/*.ts` 内; `routeOracles` → `selakovic/oracle-routing.ts` 内; `serializeValue` / `makeRecorder` / `applyIterationCap` / `nonDeterministicGlobals` / `createConsoleHook` → `common/{serializer, sandbox/capture/recording-proxy, sandbox/transforms/iteration-cap, sandbox/transforms/non-determinism, sandbox/capture/console-hook}.ts` 内 |
...
├── equivalence-checker/          # [unit] 公開 API + real vm/jsdom を叩く重いテストのみ (oracle 6 本 / routeOracles /
│   │                             #   serializer / recording-proxy / iteration-cap / non-determinism / console-hook は in-source: ADR-0007)
│   ├── common/comparison/verdict.test.ts          # deriveOverallVerdict / deriveVerdictReason (合成ロジック = 論文 audit trail)
│   ├── common/sandbox/executors/{vm,jsdom}.test.ts # executeSandboxed / executeInJsdom — real vm/jsdom/FS を叩く integration 寄り
│   └── selakovic/checker.test.ts                  # checkEquivalence (公開 API; real vm で end-to-end)
...
`src/equivalence-checker/common/comparison/oracles/*.ts` の各 in-source ブロックは 6 種の oracle (C1/C4/C5/C3+C4/C2/C6) が同階層に並列で存在し「どんな oracle があり、各々は何を見ているか」を忘れやすい。`if (import.meta.vitest)` ブロック直前の `//` コメントを仕様カタログ (観点 + 主要分岐) として機能させる好例。
