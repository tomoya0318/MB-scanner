...
`mb_scanner/mb_scanner/{equivalence,pruning,preprocessing}/models.py` と `mb-analyzer/src/contracts/{equivalence,pruning,preprocessing}-contracts.ts` の型定義は JSON 契約で一致している必要がある（詳細は [`../architecture/index.md`](../architecture/index.md)）。これを壊さないため:

- Python 側: `mb_scanner/tests/equivalence/` に subprocess を介した integration test を配置し、`dist/cli.js` を実機呼び出しで検証する
