...
`mb_scanner/domain/entities/{equivalence,pruning}.py` と `mb-analyzer/src/contracts/{equivalence,pruning}-contracts.ts` の型定義は JSON 契約で一致している必要がある（詳細は [`../architecture/index.md`](../architecture/index.md)）。これを壊さないため:
