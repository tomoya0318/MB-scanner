"""等価性検証の契約 (Port) と Node.js ランナー経由の Gateway 実装

契約 (``EquivalenceCheckerPort`` Protocol) と具象実装を同居させる (ADR-0030 feature-first)。
実装は `mb-analyzer/dist/cli.js` をサブプロセスで起動し、stdin に JSON を流し込んで
stdout から JSON を受け取る。Pydantic で検証した上でモデル型として返す。
"""
